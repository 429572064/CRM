...
class Customer(models.Model):
    ...
    consultant = models.ForeignKey('UserInfo', verbose_name="销售", related_name='customers', blank=True, null=True,on_delete=models.CASCADE,db_constraint=False )
    class_list = models.ManyToManyField('ClassList', verbose_name="已报班级",  blank=True)
    deal_date = models.DateField("报名时间", null=True, blank=True)
    ...
    def get_status(self):
        ...
        return mark_safe("<span class='btn btn-sm' style='background-color:%s;color:white'>%s</span>"%(status_color[self.status],self.get_status_display()))
...
class ClassList(models.Model):
    ...
    teachers = models.ManyToManyField('UserInfo', verbose_name="老师",db_constraint=False)
...
class ConsultRecord(models.Model):
    ...
    consultant = models.ForeignKey("UserInfo", verbose_name="跟进人", related_name='records', on_delete=models.CASCADE,db_constraint=False)
...
class Student(models.Model):
    ...
    class_list = models.ManyToManyField(verbose_name="已报班级", to='ClassList', blank=True,related_name='student')
...
class StudentStudyRecord(models.Model):
    ...
    score = models.IntegerField("本节成绩", choices=score_choices, default=-1,null=True)
